/* Luck Tester
   Static, playful, non-scientific.
   One pick per day (device-local lock).
*/
...
(function () {
  ...
  // Enable dev reset only when URL includes ?dev=1
  const devMode = new URLSearchParams(window.location.search).get("dev") === "1";
  resetBtn.style.display = devMode ? "inline-flex" : "none";
  ...
  function messageForScore(score) {
    if (score === 10) return "Perfect hit. If you’ve been waiting for a sign, this is it.";
  ...
  // ---------------------------
  // Interaction: choose a cell
  // ---------------------------
  function chooseCell(cell, icon) {
    ...
    srStatus.textContent = `Result: ${score} out of 10, ${label}.`;
  ...
  function renderGrid() {
    ...
    for (let cell = 0; cell < CELL_COUNT; cell++) {
      ...
      tile.addEventListener("click", () => chooseCell(cell, icon));

      // Keyboard support beyond default: allow Space and Enter consistently
      tile.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          chooseCell(cell, icon);
        }
      });
  ...
  function restoreIfPlayed() {
    ...
    if (last && last.date === todayKey) {
      ...
      srStatus.textContent = `Already locked for today. Your result is ${last.score} out of 10.`;
  ...
  resetBtn.addEventListener("click", () => {
    ...
    srStatus.textContent = "Dev reset used. One pick available.";
